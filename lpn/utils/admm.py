"""ADMM Solver. Modified from scico.optimize.admm.ADMM"""
...
class ADMM:
    ...
    def __init__(
        ...
        Args:
            ...
            itstat_options: A dict of named parameters to be passed to
                ...
                this parameter.
            order: The order of ADMM updates. Choose from ["123", "213", "132"].
                If "123", update x, then z, then u. If "213", update z, then x, then u. If "132",
                update x, then u, then z. Default: "123".
        """
        N = len(g_list)
        if len(C_list) != N:
        ...
        if len(rho_list) != N:
            raise Exception(
                f"len(rho_list)={len(rho_list)} not equal to len(g_list)={N}"
            )
    ...
    def norm_primal_residual(
        self, x: Optional[Union[JaxArray, BlockArray]] = None
    ) -> float:
    ...
    def u_init(
        self, x0: Union[JaxArray, BlockArray]
    ) -> List[Union[JaxArray, BlockArray]]:
        ...
        """
        u_list = [
            snp.zeros(Ci.output_shape, dtype=Ci.output_dtype) for Ci in self.C_list
        ]
        return u_list

    def x_update(self):
        r"""Update primary variable :math:`\mb{x}`."""
        self.x = self.subproblem_solver.solve(self.x)

    def z_update(self):
        r"""Update auxiliary variables :math:`\mb{z}_i`."""
    ...
    def u_update(self):
        r"""Update scaled Lagrange multipliers :math:`\mb{u}_i`."""
        for i, (rhoi, gi, Ci, zi, ui) in enumerate(
            zip(self.rho_list, self.g_list, self.C_list, self.z_list, self.u_list)
        ):
            if self.alpha == 1.0:
    ...
    def step(self):
        ...
        if self.order == "123":
            ...
            raise ValueError("order must be one of ['123', '213', '132']")
